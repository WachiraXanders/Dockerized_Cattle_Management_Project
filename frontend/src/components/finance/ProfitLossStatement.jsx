        ...
        {rows.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="bg-primary/10 rounded-lg p-3"><p className="text-xs text-primary">Total Income</p><p className="text-lg font-bold text-primary">{totals.income.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p></div>
              <div className="bg-destructive/10 rounded-lg p-3"><p className="text-xs text-destructive">Total Expenses</p><p className="text-lg font-bold text-destructive">{totals.expenses.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p></div>
              <div className={`rounded-lg p-3 ${totals.profit >= 0 ? 'bg-accent' : 'bg-gold/10'}`}><p className={`text-xs ${totals.profit >= 0 ? 'text-accent-foreground' : 'text-gold'}`}>Net Profit</p><p className={`text-lg font-bold ${totals.profit >= 0 ? 'text-accent-foreground' : 'text-gold'}`}>{totals.profit.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p></div>
            </div>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.month}>
                    ...
                    <TableCell className="text-destructive">{r.expenses.toFixed(0)}</TableCell>
                    <TableCell className={r.profit >= 0 ? 'text-primary font-medium' : 'text-gold font-medium'}>{r.profit.toFixed(0)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
