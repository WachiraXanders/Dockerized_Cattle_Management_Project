    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <p className="text-xl font-bold text-primary">Kshs {stats.totalCost.toFixed(0)}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
            ...
            <TableBody>
              {filtered.map((r) => (
                <TableRow key={r.id} className="hover:bg-muted/60">
                  ...
                  <TableCell>
                    {r.group_name ? (
                      <Badge variant="outline" className="text-xs bg-accent border-accent text-accent-foreground">
                  ...
                  <TableCell>
                    {r.total_cost ? (
                      <span className="text-sm text-primary font-medium">Kshs {r.total_cost.toFixed(2)}</span>
                    ) : '—'}
                  </TableCell>
                  <TableCell>
                    {r.stock_after_kg !== undefined && r.stock_after_kg !== null ? (
                      <span className={cn(
                        "text-sm font-medium",
                        r.stock_after_kg <= 0 ? 'text-destructive' : r.stock_after_kg < 50 ? 'text-gold' : 'text-muted-foreground'
                      )}>
                        {r.stock_after_kg} kg
                      </span>
                    ) : '—'}
