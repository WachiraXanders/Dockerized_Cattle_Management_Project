  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-md">
        <SheetHeader>
          <SheetTitle>Quick Log {!navigator.onLine && <span className="text-gold text-sm font-normal">(offline — will sync later)</span>}</SheetTitle>
        </SheetHeader>
        ...
        <div className="flex gap-1 bg-muted p-1 rounded-lg mt-4 w-fit">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium ${kind === 'milk' ? 'bg-card shadow-sm text-primary' : 'text-muted-foreground'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium ${kind === 'health' ? 'bg-card shadow-sm text-primary' : 'text-muted-foreground'}`}
          >
