    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-6">
          ...
          {/* Calculated preview */}
          {qty > 0 && selectedItem && (
            <div className={`p-3 rounded-lg border space-y-1.5 ${willTriggerAlert ? 'bg-gold/10 border-gold/20' : 'bg-muted/60 border-border'}`}>
              {headCount > 0 && (
              ...
              {selectedItem.cost_per_kg && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-primary">Kshs {(qty * selectedItem.cost_per_kg).toFixed(2)}</span>
                </div>
              )}
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Stock after</span>
                <span className={`font-semibold ${stockAfter < 0 ? 'text-destructive' : willTriggerAlert ? 'text-gold' : 'text-foreground/90'}`}>
