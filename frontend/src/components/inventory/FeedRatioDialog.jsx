  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2"><Beef className="w-5 h-5 text-primary" /> Feed Ratio — Individual Cow</SheetTitle>
        </SheetHeader>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(); }} className="space-y-4 mt-6">
          ...
          {selectedItem && amountKg > 0 && (
            <div className="bg-muted/60 rounded-lg p-3 text-sm space-y-1">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Stock remaining after</span><span className={remaining <= (selectedItem.reorder_level || 0) ? 'text-gold font-medium' : 'font-medium'}>{remaining?.toFixed(1)} kg</span></div>
            </div>
          )}
