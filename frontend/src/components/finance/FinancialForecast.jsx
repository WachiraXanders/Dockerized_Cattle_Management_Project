  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: `Total Forecast Income (${fMonths}mo)`, value: fmt(activeForecast.reduce((a, f) => a + f.forecastedIncome, 0)), color: 'text-primary', icon: TrendingUp },
          { label: `Total Forecast Expenses (${fMonths}mo)`, value: fmt(activeForecast.reduce((a, f) => a + f.forecastedExpenses, 0)), color: 'text-destructive', icon: TrendingDown },
          { label: `Forecast Profit (${fMonths}mo)`, value: fmt(activeForecast.reduce((a, f) => a + f.forecastedProfit, 0)), color: activeForecast.reduce((a, f) => a + f.forecastedProfit, 0) >= 0 ? 'text-primary' : 'text-destructive', icon: DollarSign },
          { label: `Avg Monthly Milk (L)`, value: `${(activeForecast.reduce((a, f) => a + f.forecastedMilkLiters, 0) / Math.max(fMonths, 1)).toFixed(0)}L`, color: 'text-accent-foreground', icon: Milk },
        ].map(s => (
          <Card key={s.label}>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {activeForecast.map((row, i) => (
                    <tr key={row.month} className={cn('border-b border-border', i % 2 === 0 ? 'bg-muted/60' : '')}>
                      ...
                      <td className="py-2 pr-4 text-right text-accent-foreground">{row.forecastedMilkLiters}L</td>
                      <td className="py-2 pr-4 text-right text-muted-foreground">{row.milkPrice}</td>
                      <td className="py-2 pr-4 text-right text-primary font-medium">{fmt(row.forecastedIncome)}</td>
                      <td className="py-2 pr-4 text-right text-destructive font-medium">{fmt(row.forecastedExpenses)}</td>
                      <td className={cn('py-2 text-right font-bold', row.forecastedProfit >= 0 ? 'text-primary' : 'text-destructive')}>
                ...
                <tfoot className="bg-muted">
                  <tr>
                    ...
                    <td className="py-2 pr-4 text-right text-accent-foreground font-bold">
                    ...
                    <td className={cn('py-2 text-right font-bold text-lg',
                      activeForecast.reduce((a, r) => a + r.forecastedProfit, 0) >= 0 ? 'text-primary' : 'text-destructive'
                    )}>
                ...
                <CardContent>
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      ...
                      <p className="font-bold text-primary text-sm">{fmt(s.totalIncome)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Expenses</p>
                      <p className="font-bold text-destructive text-sm">{fmt(s.totalExpenses)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Profit</p>
                      <p className={cn('font-bold text-sm', s.totalProfit >= 0 ? 'text-primary' : 'text-destructive')}>
                        {fmt(s.totalProfit)}
                      </p>
                    </div>
                  </div>
