          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3 rounded-xl bg-primary/10 border border-primary/20">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-xs text-primary">Avg Milk/Day</p>
              </div>
              <p className="text-lg font-bold text-primary">{avgMilk}L</p>
            </div>
            <div className="p-3 rounded-xl bg-accent border border-accent">
              <div className="flex items-center gap-2 mb-1">
                <Baby className="w-4 h-4 text-accent-foreground" />
                <p className="text-xs text-accent-foreground">Lactation</p>
              </div>
              <p className="text-lg font-bold text-accent-foreground">#{cattle.lactation_number || 0}</p>
            </div>
            <div className="p-3 rounded-xl bg-accent border border-accent">
              <div className="flex items-center gap-2 mb-1">
                <Calendar className="w-4 h-4 text-accent-foreground" />
                <p className="text-xs text-accent-foreground">Age</p>
              </div>
              <p className="text-sm font-bold text-accent-foreground">{getAge()}</p>
            </div>
            <div className="p-3 rounded-xl bg-gold/10 border border-gold/20">
              <div className="flex items-center gap-2 mb-1">
                <Heart className="w-4 h-4 text-gold" />
                <p className="text-xs text-gold">Health</p>
              </div>
          ...
          {currentPregnancy && (
            <div className="p-4 rounded-xl bg-accent border border-accent">
              <div className="flex items-start gap-3">
                <Baby className="w-5 h-5 text-accent-foreground mt-0.5" />
                <div className="flex-1">
                  <h3 className="text-sm font-semibold text-accent-foreground">Currently Pregnant</h3>
                  ...
                  {currentPregnancy.sire_info && (
                    <p className="text-xs text-accent-foreground">Sire: {currentPregnancy.sire_info}</p>
                  )}
                </div>
              </div>
            </div>
                ...
                <>
                  {lastCalving && (
                    <div className="p-4 rounded-xl bg-primary/10 border border-primary/20">
                      <h4 className="text-sm font-semibold text-primary mb-2">Last Calving</h4>
                      ...
                      <div key={record.id} className="p-3 rounded-lg bg-muted/60 border border-border">
                        <div className="flex items-start justify-between">
                          ...
                          <Badge className={cn("text-xs",
                            record.pregnancy_status === 'Confirmed' ? 'bg-primary/10 text-primary' :
                            record.pregnancy_status === 'Pending' ? 'bg-gold/10 text-gold' :
                  ...
                  {healthRecords.map((record) => (
                    <div key={record.id} className="p-3 rounded-lg bg-muted/60 border border-border">
                      <div className="flex items-start justify-between">
                        ...
                        <Badge className={cn("text-xs",
                          record.status === 'Resolved' ? 'bg-primary/10 text-primary' :
                          record.status === 'Ongoing' ? 'bg-gold/10 text-gold' :
            ...
            <TabsContent value="production" className="space-y-3 mt-4">
              {milkProduction.length === 0 ? (
                ...
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-xl bg-accent border border-accent">
                      <p className="text-xs text-accent-foreground mb-1">Total (30 days)</p>
                      <p className="text-xl font-bold text-accent-foreground">{totalMilk}L</p>
                    </div>
                    <div className="p-3 rounded-xl bg-primary/10 border border-primary/20">
                      <p className="text-xs text-primary mb-1">Average/Day</p>
                      <p className="text-xl font-bold text-primary">{avgMilk}L</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    {milkProduction.slice(0, 7).map((record) => (
                      <div key={record.id} className="p-3 rounded-lg bg-muted/60 border border-border flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-lg font-bold text-primary">{record.quantity_liters}L</p>
