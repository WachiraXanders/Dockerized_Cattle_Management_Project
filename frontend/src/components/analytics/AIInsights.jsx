              ...
              <CardContent>
                <div className="space-y-4">
                  {insights.recurring_health_issues.map((issue, idx) => (
                    <div key={idx} className="p-4 bg-destructive/10 rounded-lg border border-destructive/20">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className="font-mono">{issue.tag}</Badge>
                          <Badge className="bg-destructive/10 text-destructive">{issue.issue_type}</Badge>
                        </div>
                        <span className="text-sm text-destructive">
                          {issue.frequency} {issue.frequency === 1 ? 'event' : 'events'}
                        </span>
                      </div>
                      <p className="text-sm text-destructive">{issue.recommendation}</p>
                    </div>
                  ))}
                </div>
              </CardContent>
