...
**Docker (recommended)**: `docker compose up --build` runs both
containers with sensible defaults out of the box. See
[`docs/DOCKER_DEPLOYMENT.md`](./docs/DOCKER_DEPLOYMENT.md) for the full
guide, including HTTPS via Caddy and an optional Postgres profile.

**Manual**:
- Backend: any host that runs a Python ASGI app (Render, Railway, Fly.io, a
  VPS with `uvicorn`/`gunicorn`). Set the `DATABASE_URL` env var to point at
  Postgres (or any SQLAlchemy-supported database) instead of the default
  SQLite file if you outgrow SQLite — no code changes needed.
...
**Mobile**: see [`docs/MOBILE_DEPLOYMENT.md`](./docs/MOBILE_DEPLOYMENT.md)
— the app is installable as a PWA on Android/iOS once served over HTTPS.
