        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {selectedItem && (
            <div className="bg-gold/10 border border-gold/20 rounded-lg p-3 text-sm">
              <p className="text-gold font-medium">{selectedItem.name}</p>
              <p className="text-gold">Current stock: <strong>{selectedItem.total_quantity_kg} kg</strong></p>
              {selectedItem.reorder_level && (
                <p className="text-gold text-xs">Reorder level: {selectedItem.reorder_level} kg</p>
              )}
            </div>
          )}
