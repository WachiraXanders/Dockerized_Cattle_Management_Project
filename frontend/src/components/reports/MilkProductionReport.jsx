        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="bg-primary/10 rounded-lg p-4">
              <p className="text-sm text-primary">Total Production</p>
              <p className="text-2xl font-bold text-primary">{summary.totalLiters.toFixed(1)} L</p>
            </div>
            <div className="bg-accent rounded-lg p-4">
              <p className="text-sm text-accent-foreground">Used by Calves</p>
              <p className="text-2xl font-bold text-accent-foreground">{summary.totalUsedByCalves.toFixed(1)} L</p>
            </div>
            <div className="bg-accent rounded-lg p-4">
              <p className="text-sm text-accent-foreground">Net Production</p>
              <p className="text-2xl font-bold text-accent-foreground">{summary.netProduction.toFixed(1)} L</p>
            </div>
            <div className="bg-accent rounded-lg p-4">
              <p className="text-sm text-accent-foreground">Avg per Day</p>
              <p className="text-2xl font-bold text-accent-foreground">{summary.avgPerDay.toFixed(1)} L</p>
            </div>
            <div className="bg-gold/10 rounded-lg p-4">
              <p className="text-sm text-gold">Avg Fat %</p>
              <p className="text-2xl font-bold text-gold">{summary.avgFat.toFixed(2)}%</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.values(summary.lifetimeByCattle).sort((a, b) => b.total - a.total).map((cow, idx) => (
                <div key={idx} className="bg-accent border border-teal-100 rounded-lg p-3">
                  <p className="text-sm font-medium text-foreground/90">{cow.tag} {cow.name ? `- ${cow.name}` : ''}</p>
                  <p className="text-xl font-bold text-accent-foreground">{cow.total.toFixed(1)} L</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.values(summary.byCattle).map((cow, idx) => (
                <div key={idx} className="bg-card border rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-primary">{cow.total.toFixed(1)} L</p>
