...
export default function Vendors() {
  ...
  const columns = [
    ...
    { key: 'status', label: 'Status', render: (r) => <Badge className={r.status === 'Active' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}>{r.status}</Badge> },
    {
    ...
    },
    {
      ...
      render: (r) => {
        ...
        return <Badge className="bg-gold/10 text-gold">{amount.toLocaleString(undefined, { maximumFractionDigits: 0 })}</Badge>;
      },
    },
  ];
