        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-primary/10 rounded-lg p-4">
              <p className="text-sm text-primary">Total Feed Consumed</p>
              <p className="text-2xl font-bold text-primary">{summary.totalFeedKg.toFixed(1)} kg</p>
            </div>
            <div className="bg-accent rounded-lg p-4">
              <p className="text-sm text-accent-foreground">Total Cost</p>
              <p className="text-2xl font-bold text-accent-foreground">Kshs {summary.totalCost.toFixed(2)}</p>
            </div>
            <div className="bg-accent rounded-lg p-4">
              <p className="text-sm text-accent-foreground">Avg per Day</p>
              <p className="text-2xl font-bold text-accent-foreground">{summary.avgPerDay.toFixed(1)} kg</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.values(summary.byFeed).map((feed, idx) => (
                <div key={idx} className="bg-card border rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-primary">{feed.totalKg.toFixed(1)} kg</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.values(summary.byCattle).map((cow, idx) => (
                <div key={idx} className="bg-card border rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-primary">{cow.totalFeed.toFixed(1)} kg</p>
                ...
                <TableBody>
                  {filteredData.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell className="text-primary">Kshs {(record.total_cost || 0).toFixed(2)}</TableCell>
